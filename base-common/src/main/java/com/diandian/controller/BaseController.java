...
import com.diandian.util.HbUtils;
...
public abstract class BaseController<M extends BaseEntity,DTO extends BaseDTO> {
    ...
    public ApiResult list() {
        ...
        list.forEach(x -> dtoList.add(converterDTO(x,null)));
        return new ApiResult(ResultCode.SUCCESS,null,dtoList);
    }
    ...
    public ApiResult get(@PathVariable("id") String id) {
        ...
        if(model == null){
            return new ApiResult(ResultCode.PARAMS_ERROR,"对象不存在！");
        }
    ...
    public ApiResult post(@RequestBody @Valid M model) {
    ...
    public ApiResult put(@PathVariable String id, @RequestBody DTO dto) {
        M model = getService().get(id,getCurrentUser());
        model = HbUtils.deproxy(model);
    ...
    public ApiResult query(@RequestBody SearchRequest searchRequest) {
        ...
        return new ApiResult(ResultCode.SUCCESS,null,pageInfo.map(input -> converterDTO(input,null)));
    }
